...
# Direct Observation ------------------------------------------------------
get_do_week_specific <- function(week){
  ...
  do_main <- do_main %>% filter(Status != "Rejected" & weekly_reporting_round == week) %>% select(-all_of(remove_cols))
  do_goods <- do_goods %>% filter(Status != "Rejected" & weekly_reporting_round == week)
  do_Rondom_pakage1 <- do_Rondom_pakage1 %>% filter(Status != "Rejected" & weekly_reporting_round == week)
  do_Rondom_pakage2 <- do_Rondom_pakage2  %>% filter(Status != "Rejected" & weekly_reporting_round == week)
  do_Rondom_pakage3 <- do_Rondom_pakage3  %>% filter(Status != "Rejected" & weekly_reporting_round == week)
  do_Photos_Of_The_Defects_Witnessed <- do_Photos_Of_The_Defects_Witnessed  %>% filter(Status != "Rejected" & weekly_reporting_round == week)
  do_rep_If_Yes_Photos_All_Docume <- do_rep_If_Yes_Photos_All_Docume  %>% filter(Status != "Rejected" & weekly_reporting_round == week)
  ...
  write.xlsx(do_data_list, paste0("output/week_specific/Direct_observation/direct_observation_week", week,"_", today(), ".xlsx"))
...
get_form1_week_specific <- function(week){
  ...
  form1 <- form1 %>% filter(Status != "Rejected" & weekly_reporting_round == week) %>% select(-all_of(remove_cols))
  
  write.xlsx(form1, paste0("output/week_specific/Pre_distribution_form1/pre_distribution_form1_week", week,"_", today(), ".xlsx"))
...
get_form2_week_specific <- function(week){
  ...
  form2 <- form2 %>% filter(Status != "Rejected" & weekly_reporting_round == week) %>% select(-all_of(remove_cols))
  
  write.xlsx(form2, paste0("output/week_specific/Pre_distribution_form2/pre_distribution_form2_week", week,"_", today(), ".xlsx"))
